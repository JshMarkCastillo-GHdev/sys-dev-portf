...
test.describe("Mobile View - Layout Adaptation", () => {
  ...
  test("project cards stack vertically on mobile viewport", async ({
    page,
    viewport,
  }) => {
  ...
  test("project_4 Photobooth card fits within mobile viewport", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
  ...
  test("text content remains readable on mobile", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
  ...
  test("tech stack badges wrap properly on mobile", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
    ...
    const uniqueYPositions = [
      ...new Set(badgePositions.map((p) => Math.round(p.y / 5) * 5)),
    ];
    expect(uniqueYPositions.length).toBeGreaterThanOrEqual(1);
  });

  test("action buttons stack vertically on small mobile screens", async ({
    page,
    viewport,
  }) => {
  ...
  test("images scale correctly without overflow on mobile", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
  ...
  test("touch targets are adequate size on mobile", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
...
test.describe("Mobile View - Interaction", () => {
  ...
  test("horizontal scroll is not required on mobile", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
  ...
  test("cards are accessible via swipe/scroll on mobile", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
  ...
  test("show more/less toggle works on mobile touch", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
  ...
  test("no content truncation with ellipsis on critical mobile elements", async ({
    page,
    viewport,
  }) => {
    if (!viewport || viewport.width > 768) {
    ...
    for (const card of cards) {
      ...
      const buttons = card.locator(
        '[data-testid="project-actions"] button, [data-testid="project-actions"] a',
      );
      const buttonCount = await buttons.count();
