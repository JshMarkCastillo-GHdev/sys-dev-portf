...
test.describe('Cross-Browser - Visual Consistency', () => {
  ...
  test('project cards render with consistent styling across browsers', async ({ page, browserName }) => {
  ...
  test('CSS transitions work consistently across browsers', async ({ page, browserName }) => {
    const photoboothCard = page.locator('[data-testid="project-card"]', {
      has: page.locator('text=Photobooth Application'),
    });
    ...
    console.log(`[${browserName}] Transform transition: ${initialTransform} -> ${hoverTransform} -> ${finalTransform}`);
  });

  test('fonts render correctly across browsers', async ({ page, browserName }) => {
    const photoboothCard = page.locator('[data-testid="project-card"]', {
      has: page.locator('text=Photobooth Application'),
    });
  ...
  test('flexbox layout is consistent across browsers', async ({ page, browserName }) => {
    ...
    console.log(`[${browserName}] Flexbox layout verified on ${cards.length} cards`);
  });

  test('image loading handles errors consistently', async ({ page, browserName }) => {
    const photoboothCard = page.locator('[data-testid="project-card"]', {
      has: page.locator('text=Photobooth Application'),
    });
    
    const imageContainer = photoboothCard.locator('[data-testid="project-image-container"]');
    
    // Check if image or placeholder is shown
    const hasImage = await imageContainer.locator('img').isVisible().catch(() => false);
    const hasPlaceholder = await imageContainer.locator('text=Replace with real info').isVisible().catch(() => false);
    ...
    if (hasPlaceholder) {
      ...
      console.log(`[${browserName}] Placeholder rendered with:`, placeholderStyles);
    }
  });
...
test.describe('Cross-Browser - Functionality', () => {
  ...
  test('click interactions work in all browsers', async ({ page, browserName }) => {
    const photoboothCard = page.locator('[data-testid="project-card"]', {
      has: page.locator('text=Photobooth Application'),
    });
  ...
  test('keyboard navigation works in all browsers', async ({ page, browserName }) => {
    const photoboothCard = page.locator('[data-testid="project-card"]', {
      has: page.locator('text=Photobooth Application'),
    });
  ...
  test('show more/less toggle works in all browsers', async ({ page, browserName }) => {
    ...
    if (hasShowMore) {
      ...
      console.log(`[${browserName}] Description expanded: ${initialHeight}px -> ${expandedHeight}px`);
    } else {
      console.log(`[${browserName}] No show more button - description may be short`);
    }
  });

  test('link targets open correctly in all browsers', async ({ page, browserName }) => {
    const photoboothCard = page.locator('[data-testid="project-card"]', {
      has: page.locator('text=Photobooth Application'),
    });
...
test.describe('Cross-Browser - Performance', () => {
  test('page loads within acceptable time across browsers', async ({ page, browserName }) => {
    const startTime = Date.now();
    
    await page.goto('/projects');
    await page.waitForSelector('[data-testid="project-card"]', { state: 'visible', timeout: 15000 });
  ...
  test('all project cards render without layout shifts', async ({ page, browserName }) => {
