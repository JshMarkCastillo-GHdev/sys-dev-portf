...
test.describe('Project Card UI Uniformity', () => {
  ...
  test('all project cards have uniform dimensions and structure', async ({ page }) => {
    ...
    for (const card of cards) {
      ...
      await expect(card.locator('[data-testid="project-summary"]')).toBeVisible();
      
      // Content elements
      await expect(card.locator('[data-testid="project-image-container"]')).toBeVisible();
      await expect(card.locator('[data-testid="project-description"]')).toBeVisible();
      await expect(card.locator('[data-testid="project-tech-stack"]')).toBeVisible();
      
      // Footer elements
      await expect(card.locator('[data-testid="project-actions"]')).toBeVisible();
    }
  });

  test('project_4 Photobooth card matches uniform structure', async ({ page }) => {
    ...
    await expect(photoboothCard.locator('[data-testid="project-title"]')).toContainText('Photobooth Application');
    await expect(photoboothCard.locator('[data-testid="project-summary"]')).toContainText('photobooth');
    await expect(photoboothCard.locator('[data-testid="project-description"]')).toContainText('camera');
  ...
  test('image containers maintain consistent aspect ratio', async ({ page }) => {
    const cards = await page.locator('[data-testid="project-card"]').all();
  ...
  test('description expansion works uniformly across all cards', async ({ page }) => {
    const cards = await page.locator('[data-testid="project-card"]').all();
    ...
    for (const card of cards) {
      ...
      if (hasShowMore) {
        ...
        // Click to collapse
        const showLessBtn = card.locator('[data-testid="show-more-btn"]', { hasText: /Show less/i });
        await showLessBtn.click();
        await page.waitForTimeout(100);
        
        // Check collapsed text
        const collapsedHeight = await description.boundingBox().then(box => box?.height || 0);
        expect(collapsedHeight).toBeLessThan(expandedHeight);
      }
    }
  });

  test('featured badge appears only on featured projects consistently', async ({ page }) => {
    const cards = await page.locator('[data-testid="project-card"]').all();
    
    for (const card of cards) {
      const featuredBadge = card.locator('[data-testid="featured-badge"]');
      const title = await card.locator('[data-testid="project-title"]').textContent();
      
      // Check if this card should be featured based on title
      const isFeaturedProject = title?.includes('Portfolio') || title?.includes('YoloV8');
      const hasFeaturedBadge = await featuredBadge.isVisible().catch(() => false);
  ...
  test('card hover effects are consistent across all project cards', async ({ page }) => {
    const cards = await page.locator('[data-testid="project-card"]').all();
...
test.describe('Project Card Content Uniformity', () => {
  ...
  test('all project cards have complete data fields', async ({ page }) => {
    const expectedProjects = [
      { slug: 'project_1', title: 'Full Stack Developer Portfolio', featured: true },
      { slug: 'project_2', title: 'YoloV8 App with React/Vite', featured: true },
      { slug: 'project_3', title: 'Point-of-Sale Generic System', featured: false },
    ...
    for (const project of expectedProjects) {
      ...
      const title = await card.locator('[data-testid="project-title"]').textContent();
      const summary = await card.locator('[data-testid="project-summary"]').textContent();
      const description = await card.locator('[data-testid="project-description"]').textContent();
  ...
  test('project_4 Photobooth has correct tech stack', async ({ page }) => {
    ...
    const expectedTech = ['React', 'TypeScript', 'Next.js', 'Tailwind CSS', 'shadcn/ui', 'Camera API'];
